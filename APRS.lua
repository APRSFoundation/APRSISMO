...
function APRS:Object(objname, day, hour, minute, components)	-- See APRS:Posit for components
	return string.format(';%-9s*%s%s', objname, APRS:ddhhmmz(day,hour,minute), APRS:Posit(components))
end
...
--if w then print('Mic-e Type('..info.src..') '..info.platform..' '..w..' via '..info.path..' Now('..comment..')') end
			...
			elseif comment:sub(1,1) == '`' then	-- Messaging other Mic-E
				...
				local e, w = checkend(comment, '_ ')
				if e ~= nil then
					info.platform = 'Yaesu VX-8R'
					comment = comment:sub(1,e)
info.miceTrailing = w
--if w then print('Mic-e Type('..info.src..') '..info.platform..' '..w..' via '..info.path..' Now('..comment..')') end
				else
					e, w = checkend(comment,'_"')
					if e ~= nil then
						info.platform = 'Yaesu FTM-350'
						comment = comment:sub(1,e)
info.miceTrailing = w
--if w then print('Mic-e Type('..info.src..') '..info.platform..' '..w..' via '..info.path..' Now('..comment..')') end
					else
						e, w = checkend(comment,'_#')
						if e ~= nil then
							info.platform = 'Yaesu VX-8G'
							comment = comment:sub(1,e)
info.miceTrailing = w
--if w then print('Mic-e Type('..info.src..') '..info.platform..' '..w..' via '..info.path..' Now('..comment..')') end
						else
							print('Mic-E msg('..info.src..') Platform:'..info.platform)
						end
					end
				end
...
print('64+32+16+1='..s)
local testInt = bit.tonumb({'1','0','1','0','1','0','1','0'})
	print ('10..=', testInt)
local testInt2 = bit.tonumb({'0','1','0','1','0','1','0','1'})
	print ('01..=', testInt2)
]]
--[[
testTab 1       1
