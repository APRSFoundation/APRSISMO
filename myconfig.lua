...
--[[
local function tableTextFieldHandler( textField )
	return function( event )
		local fieldName = textField.name or textField.text
		local eventName = event.name or ''

		if eventName == 'touch' then
			if not textField.entryField
			and groups[fieldName]
			and groups[fieldName].len
			and groups[fieldName].len >= 0 then
				local parent = textField.row
				textField.entryField = native.newTextField( parent.x, parent.y, parent.contentWidth-4, parent.contentHeight*1.2 )
				textField.entryField.font = native.newFont( native.systemFontBold, textField.size )
				textField.entryField.text = textField.text
				textField.entryField.inputType = textField.inputType
				textField.entryField:addEventListener( "userInput", textField.touch )
				textField.row:insert(textField.entryField)
				textField.entryField.x, textField.entryField.y = textField.entryField.contentWidth/2, textField.entryField.contentHeight/2
				--textField.x, textField.y = textField.x-parent.x, textField.y-parent.y
				native.setKeyboardFocus( textField.entryField )
			end
			return true
		elseif eventName == 'userInput' then
			...
			if eventPhase == 'ended' or eventPhase == 'submitted' then	-- Elvis has left the building!
				if textField.entryField then
					if textField.text ~= textField.entryField.text then
						config[fieldName] = textField.entryField.text
						textField.changed = true
						if groups[fieldName] and groups[fieldName].action then
							if not actions[groups[fieldName].action] then actions[groups[fieldName].action] = {} end
							actions[groups[fieldName].action].action = groups[fieldName].action
							actions[groups[fieldName].action].why = fieldName
						end
					end
					textField.text = textField.entryField.text
					textField.entryField:removeSelf()
					textField.entryField = nil
					--tableView.isLocked = false
			...
			print(printableTable(fieldName,event))
		end
	end
end
]]
...
self.makeConfigScroller = function (self, scene, backAnim)
	...
	local heightCategory = 30*config.Screen.scale
...
--[[
if useID == 'Screen.Size' then
local length = #text
local left, top, right, bottom = rowValue:getStringBounds(1, length)
print(useID..':Full width is '..(right-left)..' or '..left..','..top..' -> '..right..','..bottom)
for i=1,length-1 do
left, top, right, bottom = rowValue:getStringBounds(1, length-i)
print(useID..':Minus '..i..' width is '..(right-left)..' or '..left..','..top..' -> '..right..','..bottom)
end
end
]]	
...
--[[
					rowValue:setTextSize(36*config.Screen.scale*scale)
					rowValue:setSize(guiView:getWidth()/2, heightValue)
					print('Scale Filter to '..scale)
					--rowValue:fitSize()
					--rowValue:setHeight(heightValue)
]]
			rowValue:dispose()
			rowValue = TextLabel{
				text=text,
...
DIALOG_SIZE = { Application.viewWidth * 0.95, 180}
    ...
    SceneManager:openScene("textentry_scene",
    {
        ...
        onResult = function(e)
			...
			--[[
			elseif configType ~= 'nil' then
				local rowValue = display.newText ( text, 0, 0, nil, 28)
				rowValue:setReferencePoint( display.TopLeftReferencePoint )
				fillTextSquare(row, rowValue, 1, 28, rowText.x+rowText.contentWidth)
				row:insert(rowValue)
				rowValue.x = row.contentWidth - rowValue.contentWidth - 10
				rowValue.y = 5
				rowValue:setTextColor( 0, 0, 0 )
				rowValue.name = row.id
				rowValue.touch = tableTextFieldHandler(rowValue)
				if item.isNumber then
					rowValue.inputType = 'number'
				end
				if item.isNumberF then
					rowValue.inputType = 'number'	-- Corona doesn't do 'decimal' on Android (yet)
				end
				rowValue.row = row
				--print('Parent('..tostring(rowValue)..') is '..tostring(row))
				row.value = rowValue
			end
			]]
			...
			--[[
			elseif configType ~= 'nil' then
				local text = tostring(config[row.id])
				if item.isNumberF and not string.find('%.',text) and tonumber(text) then
					text = string.format('%.1f', tonumber(text))
				end
				local rowValue = display.newText ( text, 0, 0, nil, 28)
				rowValue:setReferencePoint( display.TopLeftReferencePoint )
...
self.addGroup = function(self, group, desc)
...
self.addBoolean = function(self, group, id, desc, default, onModify)
	...
	if configXML then
		print('Config loaded from '..file..' in '..dir)
...
print("index:Accessing UNDEFINED config["..t.."]["..v.."] (cAsE sEnSiTiVe!)")
						end
						return actualConfig[t][v]	-- may return nil
					else
print("index:Accessing Table-less config["..t.."]["..v.."] (cAsE sEnSiTiVe!)")
					end
				else
print("index:Accessing UNDEFINED config["..tostring(key).."] (cAsE sEnSiTiVe!)")
				end
			end
